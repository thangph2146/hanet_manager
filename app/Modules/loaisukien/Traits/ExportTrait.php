...
namespace App\Modules\loaisukien\Traits;
...
trait ExportTrait
{
    ...
    protected function createExcelFile($data, $headers, $filters, $filename, $includeDeleted = false)
    {
        ...
        // Thêm tiêu đề
        $sheet->setCellValue('A1', 'DANH SÁCH LOẠI SỰ KIỆN');
        ...
        foreach ($data as $index => $item) {
            ...
            $sheet->setCellValue('B' . $row, $item->loai_su_kien_id);
            $sheet->setCellValue('C' . $row, $item->ten_loai_su_kien);
            $sheet->setCellValue('D' . $row, $item->ma_loai_su_kien);
