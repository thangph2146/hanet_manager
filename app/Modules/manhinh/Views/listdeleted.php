            ...
            <div class="row">
                <div class="col-12 col-md-6 mb-2 mb-md-0">
                    <a href="<?= site_url($module_name) ?>" class="btn btn-outline-primary btn-sm">
                        <i class='bx bx-arrow-back'></i> Danh sách màn hình
                    </a>
                ...
                <div class="col-12 col-md-6">
                    <form action="<?= site_url($module_name . '/listdeleted') ?>" method="get" id="search-form">
                        ...
                        <div class="input-group search-box">
                            ...
                            <select name="camera_id" class="form-select form-select-sm" style="max-width: 160px;">
                                <option value="">-- Camera --</option>
                                <?php if (isset($cameraList) && is_array($cameraList)): ?>
                                    <?php foreach ($cameraList as $camera): ?>
                                        <option value="<?= $camera->getId() ?>" <?= (isset($camera_id) && $camera_id == $camera->getId()) ? 'selected' : '' ?>>
                                            <?= esc($camera->getTenCamera()) ?>
                                        </option>
                                    <?php endforeach; ?>
                                <?php endif; ?>
                            </select>
                            <select name="template_id" class="form-select form-select-sm" style="max-width: 160px;">
                                <option value="">-- Template --</option>
                                <?php if (isset($templateList) && is_array($templateList)): ?>
                                    <?php foreach ($templateList as $template): ?>
                                        <option value="<?= $template->getId() ?>" <?= (isset($template_id) && $template_id == $template->getId()) ? 'selected' : '' ?>>
                            ...
                            <?php if (!empty($keyword) || (isset($status) && $status !== '') || (isset($camera_id) && $camera_id !== '') || (isset($template_id) && $template_id !== '')): ?>
        ...
        <?php if (!empty($keyword) || (isset($status) && $status !== '') || (isset($camera_id) && $camera_id !== '') || (isset($template_id) && $template_id !== '')): ?>
            <div class="alert alert-info m-3">
                ...
                <div class="small">
                    ...
                    <?php endif; ?>
                    <?php if (isset($camera_id) && $camera_id !== ''): ?>
                        <span class="badge bg-info me-2">Camera: <?= esc($camera_info ? $camera_info->getTenCamera() : 'Không xác định') ?></span>
                    <?php endif; ?>
                    <?php if (isset($template_id) && $template_id !== ''): ?>
                        <span class="badge bg-info me-2">Template: <?= esc($template_info ? $template_info->getTenTemplate() : 'Không xác định') ?></span>
                    <?php endif; ?>
                        ...
                        <?php if (!empty($processedData)) : ?>
                            <?php foreach ($processedData as $item) : ?>
                                <tr>
                                    ...
                                    <td><?= esc($item->getTenManHinh()) ?></td> 
                                    <td><?= esc($item->getMaManHinh()) ?></td>
                                    <td>
                                        <?php if (!empty($item->ten_camera)): ?>
                                            <?= esc($item->ten_camera) ?>
                                        <?php else: ?>
                                    ...
                                    <td>
                                        <?php if (!empty($item->ten_template)): ?>
                                            <?= esc($item->ten_template) ?>
                                        <?php else: ?>
                                            <span class="text-muted fst-italic">Chưa xác định</span>
                                        <?php endif; ?>
                                    </td>
