...
class QuanLyNguoiDung extends BaseController
{
    ...
    public function create()
    {
        ...
        if (!$this->validate($this->model->validationRules, $this->model->validationMessages)) {
            log_message('error', 'Create user - Validation failed: ' . json_encode($this->validator->getErrors()));
        ...
        try {
            ...
            if ($this->model->isAccountIdExists($data['AccountId'])) {
                ...
                return redirect()->back()
                    ->withInput()
                    ->with('error', 'Tài khoản đã tồn tại trong hệ thống');
            }

            // Kiểm tra email tồn tại
            if ($this->model->isEmailExists($data['Email'])) {
                log_message('error', 'Create user - Email already exists: ' . $data['Email']);
                return redirect()->back()
                    ->withInput()
            ...
            if ($this->model->insert($data)) {
                ...
                log_message('error', 'Create user - Insert failed: ' . json_encode($this->model->errors()));
    ...
    public function exportExcel()
    {
        $keyword = $this->request->getGet('keyword');
        $status = $this->request->getGet('status');
        $sort = $this->request->getGet('sort') ?? 'nguoi_dung_id';
        ...
        $this->createExcelFile($data, $headers, $filters, 'danh_sach_nguoi_dung');
    }
    ...
    public function exportPdf()
    {
        ...
        $this->createPdfFile($data, $filters, 'DANH SÁCH NGƯỜI DÙNG', 'danh_sach_nguoi_dung');
    }
    ...
    public function exportDeletedPdf()
    {
        ...
        $this->createPdfFile($data, $filters, 'DANH SÁCH NGƯỜI DÙNG ĐÃ XÓA', 'danh_sach_nguoi_dung_da_xoa', true);
    }
    ...
    public function exportDeletedExcel()
    {
        ...
        $this->createExcelFile($data, $headers, $filters, 'danh_sach_nguoi_dung_da_xoa', true);
    }
    ...
    protected function getSortText($sort, $order)
    {
        $sortFields = [
            ...
            'Email' => 'Email',
            'HomePhone1' => 'Điện thoại',
            'PW' => 'Mật khẩu',
            'HomePhone' => 'Điện thoại',
            'loai_nguoi_dung_id' => 'Loại người dùng',
            'mat_khau_local' => 'Mật khẩu',
            'nam_hoc_id' => 'Năm học',
            'bac_hoc_id' => 'Bậc học',
            'he_dao_tao_id' => 'Hệ đào tạo',
            'nganh_id' => 'Ngành',
