                ...
                <?php if (!empty($processedData)) : ?>
                    <?php foreach ($processedData as $item) : ?>
                        <tr>
                            ...
                            <td class="text-center">
                                <?php if (method_exists($item, 'getCheckinTypeLabel')): ?>
                                    <?= $item->getCheckinTypeLabel() ?>
                                <?php else: ?>
                                    <?= $item->checkin_type ?? 'Không xác định' ?>
                                <?php endif; ?>
                            </td>
                            <td class="text-center">
                            ...
                            <td>
                                <div class="d-flex justify-content-center gap-1 action-btn-group">
                                    <form action="<?= site_url($module_name . '/restore/' . $item->checkin_sukien_id) ?>" method="post" class="d-inline w-100 h-100">
                                        <?= csrf_field() ?>
