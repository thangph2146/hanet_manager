...
namespace App\Modules\quanlycheckoutsukien\Traits;
...
trait ExportTrait
{
        ...
        if (!empty($filters)) {
            foreach ($filters as $key => $value) {
                switch ($key) {
                    ...
                    case 'status':
                        if ($value !== '') {
                            $formattedFilters['Trạng thái'] = $value == 1 ? 'Hoạt động' : ($value == 2 ? 'Đang xử lý' : 'Vô hiệu');
                        }
                        break;
                    case 'checkout_type':
                        if (!empty($value)) {
                            ...
                            $formattedFilters['Loại check-out'] = $types[$value] ?? $value;
                        }
                        break;
                    case 'hinh_thuc_tham_gia':
                        if (!empty($value)) {
                            $formattedFilters['Hình thức tham gia'] = $value == 'offline' ? 'Trực tiếp' : 'Trực tuyến';
                        }
                        break;
                    case 'start_date':
                        if (!empty($value)) {
                            $formattedFilters['Từ ngày'] = date('d/m/Y', strtotime($value));
                        }
                        break;
                    case 'end_date':
                        if (!empty($value)) {
                            $formattedFilters['Đến ngày'] = date('d/m/Y', strtotime($value));
                        }
                        break;
                    case 'deleted':
                        if ($value === true) {
    ...
    protected function createExcelFile($data, $headers, $filters, $filename, $includeDeleted = false)
    {
        ...
        // Thêm tiêu đề
        $sheet->setCellValue('A1', 'DANH SÁCH CHECK-OUT SỰ KIỆN' . ($includeDeleted ? ' ĐÃ XÓA' : ''));
        ...
        foreach ($data as $index => $item) {
            ...
            $sheet->setCellValue($col++ . $row, $suKien ? $suKien->ten_su_kien : '');
