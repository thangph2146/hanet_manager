...
// Cấu hình URL route cho module quanlycheckoutsukien
$module_name = 'quanlycheckoutsukien';
$controller_name = 'QuanLyCheckOutSuKien';

// Định nghĩa routes cho module QuanLyCheckOutSuKien
$routes->group($module_name, ['namespace' => 'App\Modules\\' . $module_name . '\Controllers'], 
function ($routes) use ($controller_name) {
    // Route hiển thị danh sách check-out sự kiện
    ...
    // Route cập nhật trạng thái một check-out sự kiện
    $routes->post('status/(:num)', $controller_name . '::status/$1');
    
    // Route xử lý nhiều check-out sự kiện một lúc
