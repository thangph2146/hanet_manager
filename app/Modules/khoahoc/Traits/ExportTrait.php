...
trait ExportTrait
{
    ...
    protected function getExportData($criteria, $options)
    {
        ...
        $phongKhoaList = $phongKhoaModel->findAll();

        // Tạo map phòng khoa để truy cập nhanh
        $phongKhoaMap = [];
        foreach ($phongKhoaList as $phongKhoa) {
            $phongKhoaMap[$phongKhoa->phong_khoa_id] = $phongKhoa;
        }

        // Gán thông tin phòng khoa vào dữ liệu
    ...
    protected function createExcelFile($data, $headers, $filters, $filename, $includeDeleted = false)
    {
        ...
        // Thêm tiêu đề
        $sheet->setCellValue('A1', 'DANH SÁCH KHÓA HỌC');
        ...
        foreach ($data as $index => $item) {
            ...
            $sheet->setCellValue('B' . $row, $item->khoa_hoc_id);
            $sheet->setCellValue('C' . $row, $item->ten_khoa_hoc);
            $sheet->setCellValue('D' . $row, $item->nam_bat_dau);
            $sheet->setCellValue('E' . $row, $item->nam_ket_thuc);
            $sheet->setCellValue('F' . $row, !empty($item->phong_khoa) ? $item->phong_khoa->ten_phong_khoa : '');
