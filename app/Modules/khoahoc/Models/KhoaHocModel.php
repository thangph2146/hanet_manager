...
use App\Modules\khoahoc\Entities\KhoaHoc;
use App\Modules\khoahoc\Libraries\Pager;
...
class KhoaHocModel extends BaseModel
{
    ...
    protected $returnType = KhoaHoc::class;
    ...
    /**
     * Lấy tất cả bản ghi khóa học đã xóa
     *
     * @param int $limit Số lượng bản ghi trên mỗi trang
     * @param int $offset Vị trí bắt đầu lấy dữ liệu
     * @param string $sort Trường sắp xếp
    ...
    /**
     * Đếm tổng số bản ghi khóa học đã xóa
     *
     * @param array $conditions Điều kiện bổ sung
     * @return int
     */
    ...
    /**
     * Tìm kiếm khóa học dựa vào các tiêu chí
     *
    ...
    public function search(array $criteria = [], array $options = [])
    {
        ...
        // Xử lý lọc theo năm
        if (!empty($criteria['nam_bat_dau_from'])) {
            $builder->where('nam_bat_dau >=', $criteria['nam_bat_dau_from']);
        }
        
        if (!empty($criteria['nam_bat_dau_to'])) {
            $builder->where('nam_bat_dau <=', $criteria['nam_bat_dau_to']);
        }
        
        if (!empty($criteria['nam_ket_thuc_from'])) {
            $builder->where('nam_ket_thuc >=', $criteria['nam_ket_thuc_from']);
        }
        
        if (!empty($criteria['nam_ket_thuc_to'])) {
            $builder->where('nam_ket_thuc <=', $criteria['nam_ket_thuc_to']);
        }
    ...
    public function prepareValidationRules(string $scenario = 'insert', array $data = [])
    {
        ...
        // Loại bỏ validation cho khoa_hoc_id trong mọi trường hợp
        unset($this->validationRules['khoa_hoc_id']);
        
        if ($scenario === 'update' && isset($data['khoa_hoc_id'])) {
            foreach ($this->validationRules as $field => &$rules) {
                ...
                if (is_array($rules) && isset($rules['rules'])) {
                    ...
                    if (strpos($rules['rules'], 'is_unique') !== false) {
                        $rules['rules'] = str_replace('{khoa_hoc_id}', $data['khoa_hoc_id'], $rules['rules']);
                    }
                } 
                // Nếu $rules là một chuỗi
                else if (is_string($rules) && strpos($rules, 'is_unique') !== false) {
                    $rules = str_replace('{khoa_hoc_id}', $data['khoa_hoc_id'], $rules);
                }
            }
        }
    }
     ...
     * @return bool
     */
    public function isTenKhoaHocExists(string $tenKhoaHoc, ?int $excludeId = null): bool
    {
        $builder = $this->builder();
        $builder->where('ten_khoa_hoc', $tenKhoaHoc);
        $builder->where('deleted_at IS NULL');
    ...
    /**
     * Lấy danh sách khóa học theo phòng khoa
     *
     * @param int $phongKhoaId ID của phòng khoa
     * @param bool $onlyActive Chỉ lấy các khóa học đang hoạt động
     * @return array
     */
    public function getByPhongKhoa(int $phongKhoaId, bool $onlyActive = true)
    {
        ...
        $builder->orderBy('nam_bat_dau', 'DESC');
