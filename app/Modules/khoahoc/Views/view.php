...
<?= $this->endSection() ?>
<?= $this->section('title') ?>CHI TIẾT KHÓA HỌC<?= $this->endSection() ?>
...
<div class="card shadow-sm">
    <div class="card-header py-3 d-flex justify-content-between align-items-center">
        <h5 class="card-title mb-0">Chi tiết khóa học ID: <?= esc($data->khoa_hoc_id) ?></h5>
        <div class="d-flex gap-2">
            <a href="<?= site_url($module_name . '/edit/' . $data->khoa_hoc_id) ?>" class="btn btn-sm btn-primary">
            ...
            <table class="table table-bordered">
                <tbody>
                    <tr>
                        ...
                        <td><?= esc($data->khoa_hoc_id) ?></td>
                    </tr>
                    <tr>
                        <th>Tên khóa học</th>
                        <td>
                            <?php if (!empty($data->ten_khoa_hoc)): ?>
                                <?= esc($data->ten_khoa_hoc) ?>
                            <?php else: ?>
                    ...
                    <tr>
                        <th>Năm bắt đầu</th>
                        <td>
                            <?php if (!empty($data->nam_bat_dau)): ?>
                                <?= esc($data->nam_bat_dau) ?>
                            <?php else: ?>
                                <span class="text-muted fst-italic">Chưa cập nhật</span>
                            <?php endif; ?>
                        </td>
                    ...
                    <tr>
                        <th>Năm kết thúc</th>
                        <td>
                            <?php if (!empty($data->nam_ket_thuc)): ?>
                                <?= esc($data->nam_ket_thuc) ?>
                            <?php else: ?>
                                <span class="text-muted fst-italic">Chưa cập nhật</span>
                            <?php endif; ?>
                        </td>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                Bạn có chắc chắn muốn xóa khóa học <strong><?= esc($data->ten_khoa_hoc) ?></strong> không?
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Hủy</button>
                <a href="<?= site_url($module_name . '/delete/' . $data->khoa_hoc_id) ?>" class="btn btn-danger">Xóa</a>
            </div>
        </div>
    </div>
</div>
