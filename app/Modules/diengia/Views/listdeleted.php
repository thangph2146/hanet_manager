        ...
        <div class="p-3 bg-light border-bottom">
            <div class="row">
                <div class="col-12 col-md-6 mb-2 mb-md-0">
                    <?= form_open("diengia/restoreMultiple", ['id' => 'form-restore-multiple', 'class' => 'd-inline']) ?>
                    ...
                    <?= form_open("diengia/permanentDeleteMultiple", ['id' => 'form-permanent-delete-multiple', 'class' => 'd-inline']) ?>
                    <input type="hidden" name="_method" value="POST">
                    <?= csrf_field() ?>
                    <!-- Các input hidden sẽ được thêm vào bằng JavaScript -->
                ...
                <div class="col-12 col-md-6">
                    <form action="<?= site_url('diengia/listdeleted') ?>" method="get" id="search-form">
        ...
        <?php if (!empty($keyword) || (isset($status) && $status !== '')): ?>
            <div class="alert alert-info m-3">
                ...
                <div class="small">
                    ...
                    <?php endif; ?>
                    <a href="<?= site_url('diengia/listdeleted') ?>" class="text-decoration-none"><i class="bx bx-x"></i> Xóa bộ lọc</a>
                </div>
            </div>
        <?php endif; ?>
                ...
                <table id="dataTable" class="table table-striped table-hover m-0 w-100">
                    ...
                    <tbody>
                        <?php if (!empty($diengias)): ?>
                            <?php foreach ($diengias as $item): ?>
                                <tr>
                                    <td class="text-center">
                                        <div class="form-check">
                                            <input type="checkbox" name="selected_ids[]" value="<?= $item->dien_gia_id ?>" class="form-check-input checkbox-item cursor-pointer">
                                        </div>
                                    </td>
                                    <td><?= esc($item->dien_gia_id) ?></td>
                                    ...
                                    <td class="text-center">
                                        <div class="d-flex justify-content-center gap-1 action-btn-group">
                                            <form action="<?= site_url('diengia/restore/' . $item->dien_gia_id) ?>" method="post" style="display:inline;">
                                            ...
                                            data-id="<?= $item->dien_gia_id ?>" data-name="<?= $item->ten_dien_gia ?>" title="Xóa vĩnh viễn" data-bs-toggle="tooltip">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <p class="text-center">Bạn có chắc chắn muốn xóa vĩnh viễn <span id="permanent-delete-count" class="fw-bold"></span> diễn giả đã chọn?</p>
                <div class="alert alert-warning mt-3">
                    <i class='bx bx-warning me-1'></i> Lưu ý: Hành động này không thể hoàn tác!
                </div>
            </div>
...
<script>
    $(document).ready(function() {
        ...
        $('.btn-permanent-delete').on('click', function() {
            ...
            form.attr('action', base_url + '/diengia/permanentDelete/' + id);
            ...
            console.log('Permanently deleting item with return URL path:', pathAndQuery);
        ...
        $('#export-excel').on('click', function(e) {
            ...
            let exportUrl = '<?= site_url("diengia/exportDeletedExcel") ?>';
            const params = [];
            ...
            console.log('Exporting deleted diengias to Excel with URL:', exportUrl);
        ...
        $('#export-pdf').on('click', function(e) {
            ...
            let exportUrl = '<?= site_url("diengia/exportDeletedPdf") ?>';
            const params = [];
            ...
            console.log('Exporting deleted diengias to PDF with URL:', exportUrl);
