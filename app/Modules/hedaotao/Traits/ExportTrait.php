...
namespace App\Modules\hedaotao\Traits;
...
trait ExportTrait
{
    ...
    protected function createExcelFile($data, $headers, $filters, $filename, $includeDeleted = false)
    {
        ...
        // Thêm tiêu đề
        $sheet->setCellValue('A1', 'DANH SÁCH HỆ ĐÀO TẠO');
        ...
        foreach ($data as $index => $item) {
            ...
            $sheet->setCellValue('B' . $row, $item->he_dao_tao_id);
            $sheet->setCellValue('C' . $row, $item->ten_he_dao_tao);
            $sheet->setCellValue('D' . $row, $item->ma_he_dao_tao);
