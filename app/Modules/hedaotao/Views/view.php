...
<?= $this->endSection() ?>
<?= $this->section('title') ?>CHI TIẾT HỆ ĐÀO TẠO<?= $this->endSection() ?>
...
<div class="card shadow-sm">
    <div class="card-header py-3 d-flex justify-content-between align-items-center">
        <h5 class="card-title mb-0">Chi tiết hệ đào tạo ID: <?= esc($data->he_dao_tao_id) ?></h5>
        <div class="d-flex gap-2">  
            <a href="<?= site_url($module_name . '/edit/' . $data->he_dao_tao_id) ?>" class="btn btn-sm btn-primary">
            ...
            <table class="table table-bordered">
                <tbody>
                    <tr>
                        ...
                        <td><?= esc($data->he_dao_tao_id) ?></td>
                    </tr>
                    <tr>
                        <th>Tên hệ đào tạo</th>
                        <td>
                            <?php if (!empty($data->ten_he_dao_tao)): ?>
                                <?= esc($data->ten_he_dao_tao) ?>
                            <?php else: ?>
                    ...
                    <tr>
                        <th>Mã hệ đào tạo</th>
                        <td>
                            <?php if (!empty($data->ma_he_dao_tao)): ?>
                                <?= esc($data->ma_he_dao_tao) ?>
                            <?php else: ?>
                                <span class="text-muted fst-italic">Chưa cập nhật</span>
                            <?php endif; ?>
                        </td>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                Bạn có chắc chắn muốn xóa hệ đào tạo <strong><?= esc($data->ten_he_dao_tao) ?></strong> không?
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Hủy</button>
                <a href="<?= site_url($module_name . '/delete/' . $data->he_dao_tao_id) ?>" class="btn btn-danger">Xóa</a>
            </div>
        </div>
    </div>
</div>
