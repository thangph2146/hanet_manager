...
// Cấu hình URL route cho module quanlydangkysukien
$module_name = 'quanlydangkysukien';
$controller_name = 'QuanLyDangKySuKien';

// Định nghĩa routes cho module QuanLyDangKySuKien
$routes->group($module_name, ['namespace' => 'App\Modules\\' . $module_name . '\Controllers'], 
function ($routes) use ($controller_name) {
    // Route hiển thị danh sách đăng ký sự kiện
    ...
    // Route cập nhật trạng thái một đăng ký sự kiện
    $routes->post('status/(:num)', $controller_name . '::status/$1');
    
    // Route xử lý nhiều đăng ký sự kiện một lúc
    ...
    // Route xuất dữ liệu
    $routes->get('exportExcel', $controller_name . '::exportExcel');                // Xuất danh sách đăng ký sự kiện ra Excel
    $routes->get('exportDeletedExcel', $controller_name . '::exportDeletedExcel');  // Xuất danh sách đăng ký sự kiện đã xóa ra Excel
}); 