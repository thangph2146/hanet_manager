...
<?= $this->endSection() ?>
<?= $this->section('title') ?>THÙNG RÁC - LIÊN KẾT SỰ KIỆN - DIỄN GIẢ<?= $this->endSection() ?>

<?= $this->section('bread_cum_link') ?>
<?= view('components/_breakcrump', [
	'title' => 'Thùng rác - Liên kết sự kiện - diễn giả',
...
<div class="card shadow-sm">
    <div class="card-header py-3 d-flex justify-content-between align-items-center">
        <h5 class="card-title mb-0">Danh sách liên kết sự kiện - diễn giả đã xóa</h5>
        <div>
            ...
            <div class="row">
                <div class="col-12 col-md-6 mb-2 mb-md-0">
                    <a href="<?= site_url($module_name) ?>" class="btn btn-outline-primary btn-sm">
                        <i class='bx bx-arrow-back'></i> Danh sách liên kết sự kiện - diễn giả
                    </a>
                ...
                <div class="col-12 col-md-6">
                    <form action="<?= site_url($module_name . '/listdeleted') ?>" method="get" id="search-form">
                        ...
                        <div class="input-group search-box">
                            ...
                            <select name="su_kien_id" class="form-select form-select-sm" style="max-width: 160px;">
                                <option value="">-- Sự kiện --</option>
                                <?php if (isset($sukienList) && is_array($sukienList)): ?>
                                    <?php foreach ($sukienList as $sk): ?>
                                        <option value="<?= $sk['su_kien_id'] ?? $sk->su_kien_id ?? '' ?>" <?= (isset($su_kien_id) && $su_kien_id == ($sk['su_kien_id'] ?? $sk->su_kien_id ?? '')) ? 'selected' : '' ?>>
                                            <?= esc($sk['ten_su_kien'] ?? $sk->ten_su_kien ?? '') ?>
                                        </option>
                                    <?php endforeach; ?>
                                <?php endif; ?>
                            </select>
                            <select name="dien_gia_id" class="form-select form-select-sm" style="max-width: 160px;">
                                <option value="">-- Diễn giả --</option>
                                <?php if (isset($dienGiaList) && is_array($dienGiaList)): ?>
                                    <?php foreach ($dienGiaList as $dg): ?>
                                        <option value="<?= $dg->getId() ?? '' ?>" <?= (isset($dien_gia_id) && $dien_gia_id == ($dg->getId() ?? '')) ? 'selected' : '' ?>>
                                            <?= esc($dg->ten_dien_gia ?? $dg->getTenDienGia() ?? '') ?>
                                        </option>
                                    <?php endforeach; ?>
                                <?php endif; ?>
                            </select>
                            ...
                            <?php if (!empty($keyword) || (isset($su_kien_id) && $su_kien_id !== '') || (isset($dien_gia_id) && $dien_gia_id !== '')): ?>
            ...
            <div class="alert alert-info m-3">
                ...
                <div class="small">
                    ...
                    <?php if (isset($su_kien_id) && $su_kien_id !== ''): ?>
                        <span class="badge bg-secondary me-2">Sự kiện: <?= esc($su_kien_info ? $su_kien_info['ten_su_kien'] ?? $su_kien_info->ten_su_kien ?? 'Không xác định' : 'Không xác định') ?></span>
                    <?php endif; ?>
                    <?php if (isset($dien_gia_id) && $dien_gia_id !== ''): ?>
                        <span class="badge bg-info me-2">Diễn giả: <?= esc($dien_gia_info ? $dien_gia_info->ten_dien_gia ?? $dien_gia_info->getTenDienGia() ?? 'Không xác định' : 'Không xác định') ?></span>
                    <?php endif; ?>
                ...
                <table id="dataTable" class="table table-striped table-hover m-0 w-100">
                    <thead class="table-light">
                        <tr>
                            ...
                            <th width="20%" class="align-middle">Sự kiện</th>
                            <th width="20%" class="align-middle">Diễn giả</th>
                            <th width="10%" class="align-middle">Thứ tự</th>
                            <th width="20%" class="align-middle">Ngày xóa</th>
                        ...
                        <?php if (!empty($processedData)) : ?>
                            <?php foreach ($processedData as $item) : ?>
                                <tr>
                                    ...
                                    <td>
                                        <?php if (!empty($item->ten_su_kien)): ?>
                                            <?= esc($item->ten_su_kien) ?>
                                        <?php else: ?>
                                            <?= esc($item->getTenSuKien()) ?>
                                        <?php endif; ?>
                                    </td> 
                                    <td>
                                        <?php if (!empty($item->ten_dien_gia)): ?>
                                            <?= esc($item->ten_dien_gia) ?>
                                            <?php if (!empty($item->chuc_danh)): ?>
                                                <div><small class="text-muted"><?= esc($item->chuc_danh) ?></small></div>
                                            <?php endif; ?>
                                            <?php if (!empty($item->to_chuc)): ?>
                                                <div><small class="text-muted"><?= esc($item->to_chuc) ?></small></div>
                                            <?php endif; ?>
                                        <?php else: ?>
                                            <?= esc($item->getTenDienGia()) ?>
                                        <?php endif; ?>
                                    </td>
                                    ...
                                    <td>
                                        <div class="d-flex justify-content-center gap-1 action-btn-group">
                                            <button type="button" class="btn btn-success btn-sm btn-restore w-100 h-100" 
                                                    data-id="<?= $item->getId() ?>" 
                                                    data-name="ID: <?= esc($item->getId()) ?> | Sự kiện: <?= esc($item->getTenSuKien()) ?> | Diễn giả: <?= esc($item->getTenDienGia()) ?>"
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <p class="text-center">Bạn có chắc chắn muốn khôi phục bản ghi liên kết sự kiện - diễn giả:</p>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <p class="text-center">Bạn có chắc chắn muốn xóa vĩnh viễn bản ghi liên kết sự kiện - diễn giả:</p>
