...
trait ExportTrait
{
    ...
    protected function createExcelFile($data, $headers, $filters, $filename, $includeDeleted = false)
    {
        ...
        // Thêm tiêu đề
        $sheet->setCellValue('A1', 'DANH SÁCH BẬC HỌC');
        ...
        foreach ($data as $index => $item) {
            ...
            $sheet->setCellValue('B' . $row, $item->bac_hoc_id);
            $sheet->setCellValue('C' . $row, $item->ten_bac_hoc);
            $sheet->setCellValue('D' . $row, $item->ma_bac_hoc);
