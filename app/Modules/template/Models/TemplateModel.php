...
class TemplateModel extends BaseModel
{
    ...
    /**
     * Lấy tất cả bản ghi template đã xóa
     *
     * @param int $limit Số lượng bản ghi trên mỗi trang
     * @param int $offset Vị trí bắt đầu lấy dữ liệu
     * @param string $sort Trường sắp xếp
    ...
    /**
     * Đếm tổng số bản ghi template đã xóa
     *
     * @param array $conditions Điều kiện bổ sung
     * @return int
     */
    ...
    /**
     * Tìm kiếm template dựa vào các tiêu chí
     *
    ...
    public function prepareValidationRules(string $scenario = 'insert', array $data = [])
    {
        ...
        // Loại bỏ validation cho template_id trong mọi trường hợp
        unset($this->validationRules['template_id']);
            ...
            foreach ($this->validationRules as $field => &$rules) {
                ...
                if (is_array($rules) && isset($rules['rules'])) {
                    ...
                    if (strpos($rules['rules'], 'is_unique') !== false) {
                        $rules['rules'] = str_replace('{template_id}', $data['template_id'], $rules['rules']);
                    }
                } 
     ...
     * @return bool
     */
    public function isTenTemplateExists(string $tenTemplate, ?int $excludeId = null): bool
    {
        $builder = $this->builder();
