...
namespace App\Modules\template\Traits;
...
trait ExportTrait
{
    ...
    protected function createExcelFile($data, $headers, $filters, $filename, $includeDeleted = false)
    {
        ...
        // Thêm tiêu đề
        $sheet->setCellValue('A1', 'DANH SÁCH TEMPLATE');
        ...
        foreach ($data as $index => $item) {
            ...
            $sheet->setCellValue('B' . $row, $item->template_id);
            $sheet->setCellValue('C' . $row, $item->ten_template);
            $sheet->setCellValue('D' . $row, $item->ma_template);
