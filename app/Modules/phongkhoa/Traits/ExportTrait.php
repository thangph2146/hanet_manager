...
namespace App\Modules\phongkhoa\Traits;
...
trait ExportTrait
{
    ...
    protected function createExcelFile($data, $headers, $filters, $filename, $includeDeleted = false)
    {
        ...
        // Thêm tiêu đề
        $sheet->setCellValue('A1', 'DANH SÁCH PHÒNG KHOA');
        ...
        foreach ($data as $index => $item) {
            ...
            $sheet->setCellValue('B' . $row, $item->phong_khoa_id);
            $sheet->setCellValue('C' . $row, $item->ten_phong_khoa);
            $sheet->setCellValue('D' . $row, $item->ma_phong_khoa);
            $sheet->setCellValue('E' . $row, $item->ghi_chu);
