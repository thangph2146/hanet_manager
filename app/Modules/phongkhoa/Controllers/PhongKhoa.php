...
use App\Modules\phongkhoa\Traits\ExportTrait;
use App\Modules\phongkhoa\Traits\RelationTrait;
...
class PhongKhoa extends BaseController
{
    ...
    protected $moduleUrl;
    protected $title;
    protected $module_name = 'phongkhoa';
    protected $controller_name = 'PhongKhoa';
    ...
    public function __construct()
    {
        ...
        $this->model = new PhongKhoaModel();
        $this->alert = new Alert();
        
        // Thông tin module
        $this->moduleUrl = base_url($this->module_name);
        $this->title = 'Phòng Khoa';
    ...
    {
        if (empty($id)) {
            $this->alert->set('danger', 'ID phòng khoa không hợp lệ', true);
            return redirect()->to($this->moduleUrl);
        }
        ...
        if (empty($data)) {
            $this->alert->set('danger', 'Không tìm thấy dữ liệu phòng khoa', true);
            return redirect()->to($this->moduleUrl);
        }
    ...
    public function update($id = null)
    {
        ...
        $this->model->prepareValidationRules('update', ['phong_khoa_id' => $id]);
    ...
    public function delete($id = null, $backToUrl = null)
    {
        ...
        if ($this->model->delete($id)) {
            $this->alert->set('success', 'Đã xóa dữ liệu phòng khoa thành công', true);
        } else {
    ...
    /**
     * Hiển thị danh sách phòng khoa đã xóa
     */
    public function listdeleted()
    {
        ...
        if ($pager === null) {
            ...
            $pager = new \App\Modules\phongkhoa\Libraries\Pager(
                $total,
                $params['perPage'],
                $params['page']
            );
        ...
        $pager->setOnly(['keyword', 'perPage', 'sort', 'order', 'status', 'phong_khoa_id']);
        $pager->setPerPage($params['perPage']);
        $pager->setCurrentPage($params['page']);
    ...
    public function exportPdf()
    {
        ...
        $this->createPdfFile($data, $filters, 'DANH SÁCH PHÒNG KHOA', 'danh_sach_phong_khoa');
    }
    ...
    public function exportDeletedPdf()
    {
        ...
        $this->createPdfFile($data, $filters, 'DANH SÁCH PHÒNG KHOA ĐÃ XÓA', 'danh_sach_phong_khoa_da_xoa', true);
    }
    ...
    public function exportDeletedExcel()
    {
        ...
        $this->createExcelFile($data, $headers, $filters, 'danh_sach_phong_khoa_da_xoa', true);
    }
