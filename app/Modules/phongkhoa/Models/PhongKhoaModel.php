...
use App\Modules\phongkhoa\Entities\PhongKhoa;
use App\Modules\phongkhoa\Libraries\Pager;
...
class PhongKhoaModel extends BaseModel
{
    ...
    protected $returnType = PhongKhoa::class;
    ...
    /**
     * Lấy tất cả bản ghi phòng khoa đã xóa
     *
     * @param int $limit Số lượng bản ghi trên mỗi trang
     * @param int $offset Vị trí bắt đầu lấy dữ liệu
     * @param string $sort Trường sắp xếp
    ...
    /**
     * Đếm tổng số bản ghi phòng khoa
     *
    ...
    /**
     * Đếm tổng số bản ghi phòng khoa đã xóa
     *
     * @param array $conditions Điều kiện bổ sung
     * @return int
     */
    ...
    /**
     * Tìm kiếm phòng khoa dựa vào các tiêu chí
     *
    ...
    public function prepareValidationRules(string $scenario = 'insert', array $data = [])
    {
        ...
        if ($scenario === 'update' && isset($data['phong_khoa_id'])) {
            foreach ($this->validationRules as $field => &$rules) {
                ...
                if (is_array($rules) && isset($rules['rules'])) {
                    ...
                    if (strpos($rules['rules'], 'is_unique') !== false) {
                        $rules['rules'] = str_replace('{phong_khoa_id}', $data['phong_khoa_id'], $rules['rules']);
                    }
                } 
                // Nếu $rules là một chuỗi
                else if (is_string($rules) && strpos($rules, 'is_unique') !== false) {
                    $rules = str_replace('{phong_khoa_id}', $data['phong_khoa_id'], $rules);
                }
            }
        }
    }
