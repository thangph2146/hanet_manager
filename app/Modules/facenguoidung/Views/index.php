...
<?= $this->endSection() ?>
<?= $this->section('title') ?>DANH SÁCH KHUÔN MẶT NGƯỜI DÙNG<?= $this->endSection() ?>

<?= $this->section('bread_cum_link') ?>
<?= view('components/_breakcrump', [
	'title' => 'Danh sách khuôn mặt người dùng',
                ...
                <div class="col-12 col-md-6">
                    <form action="<?= site_url($module_name) ?>" method="get" id="search-form">
                        ...
                        <div class="input-group search-box">
                            <select name="nguoi_dung_id" class="form-select form-select-sm">
                                <option value="">-- Chọn người dùng --</option>
                                <?php foreach ($nguoiDungList as $nguoiDung): ?>
                                    <option value="<?= $nguoiDung->nguoi_dung_id ?>" <?= (isset($nguoi_dung_id) && $nguoi_dung_id == $nguoiDung->nguoi_dung_id) ? 'selected' : '' ?>>
                                        <?= esc($nguoiDung->getFullName()) ?> (<?= esc($nguoiDung->getEmail()) ?>)
                                    </option>
                                <?php endforeach; ?>
                            </select>
                            ...
                            <?php if (!empty($keyword) || (isset($status) && $status !== '') || (isset($nguoi_dung_id) && $nguoi_dung_id !== '')): ?>
        ...
        <?php if (!empty($keyword) || (isset($status) && $status !== '') || (isset($phong_khoa_id) && $phong_khoa_id !== '')): ?>
            <div class="alert alert-info m-3">
                ...
                <div class="small">
                    ...
                    <?php endif; ?>
                    <?php if (isset($nguoi_dung_id) && $nguoi_dung_id !== ''): ?>
                        <span class="badge bg-info me-2">Người dùng: <?= esc($nguoi_dung_info ? $nguoi_dung_info->getTenNguoiDung() : 'Không xác định') ?></span>
                    <?php endif; ?>
                ...
                <table id="dataTable" class="table table-striped table-hover m-0 w-100">
                    <thead class="table-light">
                        <tr>
                            ...
                            <th width="20%" class="align-middle">Người dùng</th>
                            <th width="20%" class="align-middle">Khuôn mặt người dùng</th>
                                ...
                                <tr>
                                    <td class="text-center">
                                        <div class="form-check">
                                            ...
                                            value="<?= $item->face_nguoi_dung_id ?>">
                                        </div>
                                    </td>
                                    <td><?= esc($item->face_nguoi_dung_id) ?></td>  
                                    <td>
                                        <div class="d-flex flex-column">
                                            <span class="fw-bold"><?= esc($item->ten_nguoi_dung) ?></span>
                                            <small class="text-muted">
                                                <?php if (!empty($item->email)): ?>
                                                    <i class="bx bx-envelope"></i> <?= esc($item->email) ?>
                                                <?php endif; ?>
                                                <?php if (!empty($item->so_dien_thoai)): ?>
                                                    <br><i class="bx bx-phone"></i> <?= esc($item->so_dien_thoai) ?>
                                                <?php endif; ?>
                                            </small>
                                        </div>
                                    </td> 
                                    <td>
                                        <?php if (!empty($item->duong_dan_anh)): ?>
                                            <div class="d-flex align-items-center">
                                                <img src="<?= base_url($item->duong_dan_anh) ?>" 
                                    ...
                                    <td>
                                        <form action="<?= site_url($module_name . '/statusMultiple') ?>" 
                                            ...
                                            <?= csrf_field() ?>
                                            <input type="hidden" name="selected_ids[]" value="<?= $item->face_nguoi_dung_id ?>">
                                    ...
                                    <td>
                                        <div class="d-flex justify-content-center gap-1 action-btn-group">
                                            <a href="<?= site_url($module_name . "/view/{$item->face_nguoi_dung_id}") ?>" class="btn btn-info btn-sm w-100 h-100" data-bs-toggle="tooltip" title="Xem chi tiết">
                                                <i class="bx bx-info-circle text-white"></i>
                                            </a>
                                            <a href="<?= site_url($module_name . "/edit/{$item->face_nguoi_dung_id}") ?>" class="btn btn-primary btn-sm w-100 h-100" data-bs-toggle="tooltip" title="Sửa">
                                                <i class="bx bx-edit"></i>
                                            </a>
                                            <button type="button" class="btn btn-danger btn-sm btn-delete w-100 h-100" 
                                                    data-id="<?= $item->face_nguoi_dung_id ?>" 
                                                    data-name="ID: <?= esc($item->face_nguoi_dung_id) ?>"
