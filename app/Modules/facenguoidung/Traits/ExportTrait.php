...
trait ExportTrait
{
    ...
    protected function getExportData($criteria, $options)
    {
        ...
        $nguoiDungModel = new \App\Modules\nguoidung\Models\NguoiDungModel();
        $nguoiDungList = $nguoiDungModel->findAll();

        // Tạo map người dùng để truy cập nhanh
        $nguoiDungMap = [];
        foreach ($nguoiDungList as $nguoiDung) {
            $nguoiDungMap[$nguoiDung->nguoi_dung_id] = $nguoiDung;
        }

        // Gán thông tin người dùng vào dữ liệu
        foreach ($data as &$item) {
            if (isset($item->nguoi_dung_id)) {
                $nguoiDung = isset($nguoiDungMap[$item->nguoi_dung_id]) ? $nguoiDungMap[$item->nguoi_dung_id] : null;
    ...
    protected function createExcelFile($data, $headers, $filters, $filename, $includeDeleted = false)
    {
        ...
        // Thêm tiêu đề
        $sheet->setCellValue('A1', 'DANH SÁCH KHUÔN MẶT NGƯỜI DÙNG');
        ...
        foreach ($data as $index => $item) {
            ...
            $sheet->setCellValue('B' . $row, $item->face_nguoi_dung_id);
            $sheet->setCellValue('C' . $row, $item->ten_nguoi_dung);
            $sheet->setCellValue('D' . $row, $item->duong_dan_anh);
