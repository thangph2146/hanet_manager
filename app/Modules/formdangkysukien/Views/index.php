...
<?= $this->endSection() ?>
<?= $this->section('title') ?>DANH SÁCH FORM ĐĂNG KÝ SỰ KIỆN<?= $this->endSection() ?>
...
<?= view('components/_breakcrump', [
	...
	'breadcrumbs' => [
		['title' => 'Quản lý Form đăng ký sự kiện', 'url' => site_url($module_name)],
...
<div class="card shadow-sm">
    <div class="card-header py-3 d-flex justify-content-between align-items-center">
        <h5 class="card-title mb-0">Danh sách form đăng ký sự kiện</h5>
        <div>
                    ...
                    <form action="<?= site_url($module_name) ?>" method="get" id="search-form">
                        ...
                        <div class="d-flex flex-column gap-2">
                            <div class="input-group search-box">
                                ...
                                <?php if (!empty($keyword) || isset($_GET['su_kien_id']) || isset($_GET['status']) || isset($_GET['hien_thi_cong_khai']) || isset($_GET['bat_buoc_dien'])): ?>
                            ...
                            <div class="d-flex flex-wrap gap-2">
                                ...
                                <!-- Bộ lọc bắt buộc điền -->
                                <select name="bat_buoc_dien" class="form-select form-select-sm" style="max-width: 200px;" onchange="this.form.submit()">
                                    <option value="">Tất cả</option>
                                    <option value="1" <?= (isset($_GET['bat_buoc_dien']) && $_GET['bat_buoc_dien'] === '1') ? 'selected' : '' ?>>Bắt buộc điền</option>
                                    <option value="0" <?= (isset($_GET['bat_buoc_dien']) && $_GET['bat_buoc_dien'] === '0') ? 'selected' : '' ?>>Không bắt buộc điền</option>
                                </select>
                            </div>
                        </div>
                    </form>
                ...
                <div class="small">
                    ...
                    <?php if (isset($_GET['status']) && $_GET['status'] !== ''): ?>
                        <span class="badge bg-primary me-2">Trạng thái: 
                            <?= $_GET['status'] === '1' ? 'Hoạt động' : 'Không hoạt động' ?>
                        </span>
                    <?php endif; ?>
                    <?php if (isset($_GET['hien_thi_cong_khai']) && $_GET['hien_thi_cong_khai'] !== ''): ?>
                        <span class="badge bg-primary me-2">Hiển thị: 
                            <?= $_GET['hien_thi_cong_khai'] === '1' ? 'Hiển thị công khai' : 'Không hiển thị công khai' ?>
                        </span>
                    <?php endif; ?>
                    <?php if (isset($_GET['bat_buoc_dien']) && $_GET['bat_buoc_dien'] !== ''): ?>
                        <span class="badge bg-primary me-2">Bắt buộc điền: 
                            <?= $_GET['bat_buoc_dien'] === '1' ? 'Bắt buộc điền' : 'Không bắt buộc điền' ?>
                        </span>
                    <?php endif; ?>
                ...
                <table id="dataTable" class="table table-striped table-hover m-0 w-100">
                    <thead class="table-light">
                        <tr>
                            ...
                            <th width="15%" class="align-middle">Tên form</th>
                            <th width="15%" class="align-middle">Sự kiện</th>
                            <th width="10%" class="align-middle">Số trường</th>
                            <th width="10%" class="align-middle">Bắt buộc điền</th>
                            <th width="10%" class="align-middle">Hiển thị công khai</th>
                            <th width="10%" class="align-middle">Số lần sử dụng</th>
                        ...
                        <?php if (!empty($processedData)) : ?>
                            <?php foreach ($processedData as $item) : ?>
                                <tr>
                                    ...
                                    <td><?= esc($item->getFormId()) ?></td>
                                    <td><?= esc($item->getTenForm()) ?></td>
                                    <td><?= esc($item->getTenSuKien()) ?></td>
                                    <td><?= esc($item->countFields()) ?> (<?= esc($item->countRequiredFields()) ?> bắt buộc)</td>
                                    <td>
                                        <span class="badge <?= $item->getBatBuocDien() ? 'bg-success' : 'bg-secondary' ?>">
                                            <?= $item->getBatBuocDien() ? 'Bắt buộc' : 'Không bắt buộc' ?>
                                        </span>
                                    </td>
                                    <td>
                                        <span class="badge <?= $item->getHienThiCongKhai() ? 'bg-primary' : 'bg-secondary' ?>">
                                            <?= $item->getHienThiCongKhai() ? 'Công khai' : 'Không công khai' ?>
                                        </span>
                                    </td>
                                    <td><?= esc($item->getSoLanSuDung()) ?></td>
                                    <td>
                                        <div class="dropdown">
                                            <button class="btn btn-sm dropdown-toggle 
                                                <?= $item->getStatus() ? 'btn-success' : 'btn-danger' ?>"
                                                type="button" data-bs-toggle="dropdown" aria-expanded="false">
                                                <?= $item->getStatusText() ?>
                                            </button>
                                            <ul class="dropdown-menu">
                                                <li><a class="dropdown-item <?= $item->getStatus() == 1 ? 'active' : '' ?>" 
                                                    href="<?= site_url($module_name . '/updateStatus/' . $item->getId() . '/1?return_url=' . current_url()) ?>">
                                                    Hoạt động
                                                </a></li>
                                                <li><a class="dropdown-item <?= $item->getStatus() == 0 ? 'active' : '' ?>"
                                    ...
                                    <td>
                                        <div class="d-flex justify-content-center gap-1 action-btn-group">
                                            ...
                                            <button type="button" class="btn btn-danger btn-sm btn-delete w-100 h-100" 
                                                    data-id="<?= $item->getId() ?>" 
                                                    data-name="form '<?= esc($item->getTenForm()) ?>'"
