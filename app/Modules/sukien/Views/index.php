                ...
                <div class="col-12 col-md-6">
                    <form action="<?= site_url($module_name) ?>" method="get" id="search-form">
                        ...
                        <div class="input-group search-box">
                            ...
                            <select name="loai_su_kien_id" class="form-select form-select-sm" style="max-width: 160px;">
                                <option value="">-- Loại sự kiện --</option>
                                <?php if (isset($loaiSuKienList) && is_array($loaiSuKienList)): ?>
                                    <?php foreach ($loaiSuKienList as $lsk): ?>
                                        <option value="<?= $lsk->getId() ?>" <?= (isset($loai_su_kien_id) && $loai_su_kien_id == $lsk->getId()) ? 'selected' : '' ?>>
                                            <?= esc($lsk->getTenLoaiSuKien()) ?>
                                        </option>
                                    <?php endforeach; ?>
                                <?php endif; ?>
                            </select>
                            ...
                            <?php if (!empty($keyword) || (isset($status) && $status !== '') || (isset($loai_su_kien_id) && $loai_su_kien_id !== '')): ?>
            ...
            <div class="alert alert-info m-3">
                ...
                <div class="small">
                    ...
                    <?php if (isset($loai_su_kien_id) && $loai_su_kien_id !== ''): ?>
                        <span class="badge bg-info me-2">Loại sự kiện: <?= esc($loai_su_kien_info ? $loai_su_kien_info->getTenLoaiSuKien() : 'Không xác định') ?></span>
                    <?php endif; ?>
                ...
                <table id="dataTable" class="table table-striped table-hover m-0 w-100">
                    <thead class="table-light">
                        <tr>
                            ...
                            <th width="15%" class="align-middle">Poster</th>
                        ...
                        <?php if (!empty($processedData)) : ?>
                            <?php foreach ($processedData as $item) : ?>
                                <tr>
                                    ...
                                    <td>
                                        <?php 
                                        $poster = $item->getSuKienPoster();
                                        if ($poster && isset($poster['url'])): 
                                        ?>
                                            <img src="<?= esc($poster['url']) ?>" 
                                                 alt="Poster <?= esc($item->ten_su_kien) ?>" 
                                                 class="img-thumbnail" 
                                    ...
                                    <td>
                                        <div><i class="bx bx-calendar me-1"></i> <?= date('d/m/Y', strtotime($item->thoi_gian_bat_dau)) ?></div>
                                        <div><i class="bx bx-time me-1"></i> <?= date('H:i', strtotime($item->thoi_gian_bat_dau)) ?> - <?= date('H:i', strtotime($item->thoi_gian_ket_thuc)) ?></div>
                                    </td>
                                    <td>
                                        <?= esc($item->dia_diem) ?>
                                        <?php if (!empty($item->dia_chi_cu_the)): ?>
                                            <div class="small text-muted"><?= esc($item->dia_chi_cu_the) ?></div>
                                        <?php endif; ?>
                                    </td>
                                    <td>
                                        <?php if (!empty($item->ten_loai_su_kien)): ?>
                                            <?= esc($item->ten_loai_su_kien) ?>
                                        <?php else: ?>
                                    ...
                                    <td>
                                        <form action="<?= site_url($module_name . '/statusMultiple') ?>" 
                                            ...
                                            <?= csrf_field() ?>
                                            <input type="hidden" name="selected_ids[]" value="<?= $item->su_kien_id ?>">
                                    ...
                                    <td>
                                        <div class="d-flex justify-content-center gap-1 action-btn-group">
                                            <a href="<?= site_url($module_name . "/view/{$item->su_kien_id}") ?>" class="btn btn-info btn-sm w-100 h-100" data-bs-toggle="tooltip" title="Xem chi tiết">
                                                <i class="bx bx-info-circle text-white"></i>
                                            </a>
                                            <a href="<?= site_url($module_name . "/edit/{$item->su_kien_id}") ?>" class="btn btn-primary btn-sm w-100 h-100" data-bs-toggle="tooltip" title="Sửa">
                                                <i class="bx bx-edit"></i>
                                            </a>
                                            <button type="button" class="btn btn-danger btn-sm btn-delete w-100 h-100" 
                                                    data-id="<?= $item->su_kien_id ?>" 
                                                    data-name="ID: <?= esc($item->su_kien_id) ?> - <?= esc($item->ten_su_kien) ?>"
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <p class="text-center">Bạn có chắc chắn muốn xóa <span id="selected-count" class="fw-bold"></span> sự kiện đã chọn?</p>
                <div class="alert alert-warning mt-3">
                    <i class="bx bx-info-circle me-1"></i> Dữ liệu sẽ được chuyển vào thùng rác và có thể khôi phục.
                </div>
            </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <p class="text-center">Bạn có chắc chắn muốn thay đổi trạng thái của <span id="status-count" class="fw-bold"></span> sự kiện đã chọn?</p>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Hủy</button>
